...
bool YoloObjectDetector::readParameters() {
  ...
  nodeHandle_.param("image_view2/enable_opencv", viewImage_, true);
  nodeHandle_.param("image_view2/wait_key_delay", waitKeyDelay_, 3);
  nodeHandle_.param("image_view2/enable_console_output", enableConsoleOutput_, false);
...
void YoloObjectDetector::cameraCallback(const sensor_msgs::ImageConstPtr& msg) {
  ...
  //cout << "received" << endl;
  cur = ros::Time::now();
  elapsed = cur - msg->header.stamp;
  cout << "sub_image " << msg->header.seq << " " << msg->header.stamp << " " <<  ros::Time::now() << " " << elapsed << endl;
...
void* YoloObjectDetector::publishInThread() {
  ...
  if (num > 0 && num <= 100) {
    ...
    msg.header.frame_id = cam_frame_id;
    msg.count = num;
    ...
    boundingBoxesResults_.header.seq = cnt++;
    boundingBoxesResults_.image_header = headerBuff_[(buffIndex_ + 1) % 3];
    boundingBoxesPublisher_.publish(boundingBoxesResults_); //here pub bounding boxes
    elapsed = boundingBoxesResults_.header.stamp - cur;
    cout << "pub_bound " << boundingBoxesResults_.image_header.seq << " " << boundingBoxesResults_.header.seq << " " << boundingBoxesResults_.header.stamp << endl;
  } else {
    darknet_ros_msgs::ObjectCount msg;
    msg.header.stamp = ros::Time::now();
